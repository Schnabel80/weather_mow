...
def update_states(hass: MockHass, tick: dict, mower_ha_state: str) -> None:
    ...
    hass.states.set(
        "sun.sun", "above_horizon" if elev > 0 else "below_horizon", attributes={"elevation": elev}
    )
...
def fire_mow_state_change(
    coordinator: WeatherMowCoordinator, old_state: str, new_state: str, time_utc
) -> None:
...
async def run(past_days: int = 14, force_refresh: bool = False) -> list[dict]:
    ...
    for i, tick in enumerate(ticks):
        ...
        except Exception as exc:  # noqa: BLE001 — Sim soll bei Einzelfehler weiterlaufen
...
def main() -> None:
    ...
    parser.add_argument("--refresh", action="store_true", help="Force re-download of weather data")
